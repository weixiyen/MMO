UI.module('feed.topic', function(){
    ...
    var menu = UI.module.menu('#menu'),
        ...
        sport = $('#TOPIC').attr('title'),
        channel = $('#TOPIC').attr('rel');
    
    news.loadSport(sport, 0, 10);
    ...
    $('#delete-topic').bind('click', function(){
        ...
        UI.confirm(question, function(){
            feed.removeTopic(topic_id, function(){
                location.href = '/' + sport + '#/channel/' + channel;
            });
        }); 
    });
    ...
    -------------------------------------------------*/
    var request = {
        channels: ['topic:' + topic_id, 'news:' + sport]
    };
